...
# OpenAI client for HuggingFace inference endpoint
from openai import OpenAI  # type: ignore # pyrefly: ignore [missing-import]
...
HF_TOKEN = "YOUR_HUGGINGFACE_TOKEN_HERE"  # <-- You need to put your HF token here
MODEL_ID = "AI4Chem/ChemLLM-7B-Chat"

# using openai client to contact huggingface inference endpoints
client = OpenAI(
...
if prompt := st.chat_input("Ask about optimization, parameters, or the BMR..."):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            st.error(f"API Error: {e}. Please wait a moment.")
        except Exception as e:
            st.error(f"Error: {e}")