...
# Initialize local OCR engine (PPStructure for document layout analysis)
@st.cache_resource
def get_table_engine():
    # Only show logs if necessary
    return PPStructure(show_log=False, layout=True)

def extract_pdf_with_paddleocr(uploaded_file, file_state_key, text_state_key, name="Document"):
    if uploaded_file.name != st.session_state[file_state_key]:
        ...
        full_text = ""
        progress_bar = st.progress(0, text=f"Initializing {name} parsing...")
        ...
        for i in range(len(pdf)):
            ...
            progress_bar.progress(i / len(pdf), text=f"Analyzing {name} Page {i+1} with PaddleOCR PP-Structure...")
            ...
            try:
                ...
                # PPStructure returns a list of dictionaries with type, bbox, and res
                for region in result:
                    region_type = region.get('type', 'Unknown')
                    res = region.get('res', [])
                    
                    if region_type == 'table':
                        # Extracts table as HTML string
                        html_table = res.get('html', '')
                        if html_table:
                            full_text += f"\n[Table Detected]\n{html_table}\n"
                    else:
                        # Extract standard text lines and join them
                ...
                full_text += f"\n[Error extracting Page {i+1} with PaddleOCR: {e}]\n"
        ...
        st.success(f"✅ {name} PaddleOCR PP-Structure Extraction Complete")
    else:
...
with col_ros:
    if uploaded_ros_file:
        extract_pdf_with_paddleocr(uploaded_ros_file, "last_uploaded_ros_file", "ros_text", "ROS")

with col_bmr:
    if uploaded_file:
        extract_pdf_with_paddleocr(uploaded_file, "last_uploaded_file", "bmr_text", "BMR")

st.markdown("---")

# Show text if it exists
if st.session_state.bmr_text or st.session_state.ros_text:
    with st.expander("📝 View Extracted Text Layout (Paddle OCR)"):
        if st.session_state.ros_text:
            st.write("### ROS Text")
            st.markdown(st.session_state.ros_text, unsafe_allow_html=True)  # Markdown to render HTML tables
        if st.session_state.bmr_text:
            st.write("### BMR Text")
            st.markdown(st.session_state.bmr_text, unsafe_allow_html=True)
