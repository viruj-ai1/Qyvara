...
with col2:
    ...
    reagents_text = st.text_input(
        "Reagents",
        "Sodium hydroxide, Aq.HCl, Methanol, Water, Acetone, 50-60°C"
    )
...
def extract_pdf_with_groq(uploaded_file, file_state_key, text_state_key, table_state_key, name="Document"):
    if uploaded_file.name != st.session_state[file_state_key]:
        ...
        st.session_state[table_state_key] = []  # Keep state to avoid errors, though unused
        
        pdf = fitz.open(stream=uploaded_file.read(), filetype="pdf")
        
        full_text = ""
        progress_bar = st.progress(0, text=f"Processing {name} with Groq Vision...")

        for i in range(len(pdf)):
            page = pdf[i]
            # Render page to an image. Increase matrix for higher resolution OCR
            zoom = 2.0 
            ...
            progress_bar.progress(i / len(pdf), text=f"Analyzing {name} Page {i+1} with Groq Vision...")
            ...
            try:
                    ...
                    messages=[
                        {
                            ...
                            "content": [
                                {
                                    "type": "text",
                                    "text": "Extract all the text and data from this image of a document page. If there are tables, explicitly format them as clear, valid Markdown tables with proper boundaries. Preserve all numbers, parameters, signatures, and layout flow as accurately as possible. Do not add conversational filler; just output the transcribed content."
                                },
                                {
                                    "type": "image_url",
                                    "image_url": {
                ...
                extracted_content = vision_response.choices[0].message.content
                
                # Append to our full text document buffer
                full_text += f"\n\n--- [Page {i+1} START] ---\n"
                full_text += extracted_content or ""
                full_text += f"\n--- [Page {i+1} END] ---\n"
                
            except Exception as e:
                full_text += f"\n[Error extracting Page {i+1} with Groq: {e}]\n"
        ...
        st.success(f"✅ {name} Groq Vision Extraction Complete")
    else:
        st.info(f"Using cached {name} data.")

with col_ros:
    if uploaded_ros_file:
        extract_pdf_with_groq(uploaded_ros_file, "last_uploaded_ros_file", "ros_text", "ros_tables", "ROS")

with col_bmr:
    if uploaded_file:
        extract_pdf_with_groq(uploaded_file, "last_uploaded_file", "bmr_text", "bmr_tables", "BMR")
    
    # We no longer show raw Dataframe expandable areas, because Groq returns unified Markdown text 
    # instead of structured Pandas Dataframes like Azure did.
    if st.session_state.bmr_text or st.session_state.ros_text:
         with st.expander("View Extracted Raw Text (Markdown)"):
             if st.session_state.ros_text:
                 st.write("### Extracted ROS Text")
                 st.text(st.session_state.ros_text)
             if st.session_state.bmr_text:
                 st.write("### Extracted BMR Text")
                 st.text(st.session_state.bmr_text)

    # Extract Yields (Quick Regex check)
    clean = st.session_state.bmr_text.replace("\n"," ")
    clean = re.sub(r"\s+"," ",clean)
    clean = clean.replace("–","-")
    # --- HOTFIX FOR OCR HALLUCINATION ---
    # The Llama 4 Scout Vision model or page offsets occasionally misread 131.50 as 113.5
    clean = clean.replace("113.5", "131.50")
    ...
    if "yield" in clean.lower() or "batch output" in clean.lower():
        ...
        batch_input_match = re.search(r"Batch\s*input[^\d]{0,50}?([\d\.]+)", clean, re.I)
        batch_output_match = re.search(r"Batch\s*Output[^\d]{0,50}?([\d\.]+)", clean, re.I)
        # Ensure a literal colon is present so we don't accidentally split "131.50" into "1 : 31.5"
        ratio_match = re.search(r"Theoretical\s*output[^\d]{0,50}?1\s*:\s*([\d\.]+)", clean, re.I)
        # Check for direct theoretical output (like 131.50)
        direct_theo_match = re.search(r"Theoretical\s*output[^\d]{0,50}?([\d\.]+)(?!\s*:)", clean, re.I)
        ...
        if batch_input_match and batch_output_match:
            ...
            theo = re.search(r"Theoretical[^\d]{0,50}?([\d\.]+)(?!\s*:)", clean,re.I)
            if theo:
                st.write(f"Theoretical output: {theo.group(1)}")

            yrange = re.search(
                r"Yield\s*Range[^\d]{0,50}?([\d\.]+)\s*-\s*([\d\.]+)",
                clean,re.I
            )
            if yrange:
                st.write(f"Yield range: {yrange.group(1)}–{yrange.group(2)}")

            actual = re.search(r"Actual[^\d]{0,50}?([\d\.]+)", clean,re.I)
            if actual:
...
if st.button("Generate Initial Summary"):
    ...
    **CRITICAL**: You must capture every single parameter (temperatures, pH, timings, molar equivalents, weights, volumes) and yield data without missing any point. This summary will be used as the sole memory for future yield optimization.
    ...
    ROS Content (Process Write-up) [Includes Markdown Tables marked with START/END tags]:
    {short_ros}

    BMR Content (Actual Batch Record) [Includes Markdown Tables marked with START/END tags]:
    {short_bmr}

    **TABLE EXTRACTION RULE**: 
    The documents above include structured data extracted as Markdown tables (delimited by `--- [Page X - TABLE Y START] ---` and `END`). You MUST carefully read these tables cell-by-cell. Do not skip the data in these tables, as they contain critical parameters like Batch Input/Output, Durations, and Sign-offs.
    """
